...
function svgToPngDataUrl(svgStr, w, h, scale) {
  return new Promise((resolve, reject) => {
    ...
    img.onerror = () => { URL.revokeObjectURL(url); reject(new Error("chart raster failed")); };
    img.src = url;
  });
}
...
async function exportXLSX() {
  ...
  const H = m ? Math.round(parseFloat(m[2])) : 500;
  try {
    let chart = null;
    try { chart = await svgToPngDataUrl(svg, W, H, 2); } catch (_) { /* table-only fallback */ }
    const resp = await fetch("/api/export/xlsx", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        filename, sheet_name: t("sch.sheetTasks"), columns, rows,
        chart_png_base64: chart, chart_sheet_name: t("sch.sheetChart"), chart_w: W, chart_h: H,
      }),
    });
